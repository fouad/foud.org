...
function AboutSection(props) {
  ...
  return (
    <section {...props}>
      ...
      <p
        className={clsx(
          'mt-2 text-base leading-7 text-slate-400',
          !isExpanded && 'lg:line-clamp-4'
        )}
      >
        This is a blog dedicated to culinary artistry and appreciation of chefs. 
      </p>
      {/* {!isExpanded && (
        <button
          type="button"
    ...
    <>
      <header className="bg-black lg:fixed lg:inset-y-0 lg:left-0 lg:flex lg:w-112 lg:items-start lg:overflow-y-auto xl:w-120">
        <div className="hidden lg:sticky lg:top-0 lg:flex lg:w-16 lg:flex-none lg:items-center lg:whitespace-nowrap lg:py-12 lg:text-sm lg:leading-7 lg:[writing-mode:vertical-rl]">
          <span className="font-mono text-slate-50">Curated by</span>
          <span className="mt-6 flex gap-6 font-bold text-slate-50">
        ...
        <div className="relative z-10 mx-auto px-4 pb-4 pt-10 sm:px-6 md:max-w-2xl md:px-4 lg:min-h-full lg:flex-auto lg:border-x lg:border-slate-800 lg:py-12 lg:px-8 xl:px-12">
          <Link
            href="/"
            className="relative mx-auto block w-48 overflow-hidden rounded-lg bg-slate-200 shadow-xl shadow-slate-700 sm:w-64 sm:rounded-xl lg:w-auto lg:rounded-2xl"
            aria-label="Homepage"
          >
            <Image
              className="w-full"
          ...
          <div className="mt-10 text-center lg:mt-12 lg:text-left">
            <p className="font-serif text-xl font-extralight text-slate-50">
              <Link href="/">foud</Link>
            </p>
            <p className="mt-3 text-lg font-medium leading-8 text-slate-400">
              Admiring function and form in food.
            </p>
          </div>
          <AboutSection className="mt-12 hidden lg:block" />
        </div>
      </header>
      <main className="border-t border-gray-200 bg-black lg:relative lg:pb-28 lg:ml-112 lg:border-t-0 xl:ml-120">
      ...
      <footer className="border-t border-slate-200 bg-slate-50 py-10 pb-40 sm:py-16 sm:pb-32 lg:hidden">
        <div className="mx-auto px-4 sm:px-6 md:max-w-2xl md:px-4">
          <AboutSection />
          <h2 className="mt-8 flex items-center font-mono text-sm font-medium leading-7 text-slate-50">
            <PersonIcon className="h-3 w-auto fill-slate-300" />
            <span className="ml-2.5">Hosted by</span>
          </h2>
          <div className="mt-2 flex gap-6 text-sm font-bold leading-7 text-slate-50">
            {hosts.map((host, hostIndex) => (
              <Fragment key={host}>
                {hostIndex !== 0 && (
                  <span aria-hidden="true" className="text-slate-400">
